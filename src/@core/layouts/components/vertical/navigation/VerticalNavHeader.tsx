    ...
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft() }}>
      {userVerticalNavMenuBranding ? (
        ...
        <Box sx={{ paddingY: 10, marginBottom: 12 }}>
          <Link href={user?.role?.code === 'ADMIN' ? '/dashboard' : '/dashboard/product'} passHref>
            <StyledLink>
              <div style={{ width: 200, height: 'auto' }}>
                <Image
                  src={'/images/zipaway-logo.png'}
                  width={!navCollapsed ? 200 : navHover ? 200 : 34}
                  height={!navCollapsed ? 100 : navHover ? 100 : 34}
                  style={{ objectFit: 'contain', transition: '.3s all ease' }}
                  alt='logo'
                />
              </div>
            </StyledLink>
