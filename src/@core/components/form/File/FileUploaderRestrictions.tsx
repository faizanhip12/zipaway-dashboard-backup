...
function readableBytes(bytes: number) {
  ...
  // @ts-ignore
  return Math.ceil((bytes / Math.pow(1024, 2)).toFixed(2) * 1) + ' ' + size
}
...
const FileUploaderRestrictions = ({
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    maxSize,
    accept,
    // validator: imageWidthValidator,
    onDrop: (acceptedFiles: File[]) => {
      setFiles(acceptedFiles.map((file: File) => Object.assign(file)))
    },
    onDropRejected: () => {
      toast.error(`You can only upload ${maxFiles} files & maximum size of ${readableBytes(maxSize)} Image.`)
    }
  })
  ...
  const renderFilePreviewWithUrl = (value: string) => {
    if (value) {
      return <img width={38} height={38} style={{ objectFit: 'contain' }} alt='file' src={value} />
    }
  }
      ...
      <div {...getRootProps({ className: 'dropzone' })}>
        ...
        <Box sx={{ display: 'flex', flexDirection: ['column', 'column', 'row'], alignItems: 'center' }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', textAlign: ['center', 'center', 'inherit'] }}>
            ...
            <Typography color='textSecondary'>
              Max {maxFiles} files and max size of {readableBytes(maxSize)}
            </Typography>
          </Box>
        </Box>
      </div>
